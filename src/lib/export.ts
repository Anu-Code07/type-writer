import { escapeHtml, htmlToMarkdown, htmlToPlainText, plainTextToHtml } from "@/lib/richText";
import type { WritingBook, WritingDocument } from "@/lib/indexeddb";
import { createWritingPdf } from "@/lib/pdfExport";

export type ExportFormat = "txt" | "markdown" | "pdf";

const safeFileName = (title: string, extension: ExportFormat) => {
  const slug = title
    .trim()
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, "-")
    .replace(/(^-|-$)/g, "");
  const suffix = extension === "markdown" ? "md" : extension;

  return `${slug || "untitled"}.${suffix}`;
};

const downloadBlob = (blob: Blob, fileName: string) => {
  const url = URL.createObjectURL(blob);
  const anchor = window.document.createElement("a");

  anchor.href = url;
  anchor.download = fileName;
  anchor.click();
  URL.revokeObjectURL(url);
};

const buildMarkdown = (document: WritingDocument) => {
  const title = document.title.trim() || "Untitled";
  return `# ${title}\n\n${htmlToMarkdown(document.content)}`;
};

const orderedBookDocuments = (book: WritingBook, documents: WritingDocument[]) =>
  book.documentIds
    .map((documentId) => documents.find((document) => document.id === documentId))
    .filter((document): document is WritingDocument => Boolean(document));

const buildBookContent = (book: WritingBook, documents: WritingDocument[], format: Exclude<ExportFormat, "pdf">) => {
  const chapters = orderedBookDocuments(book, documents);

  if (format === "markdown") {
    return [`# ${book.title}`, ...chapters.map((document) => `## ${document.title}\n\n${htmlToMarkdown(document.content)}`)].join(
      "\n\n---\n\n",
    );
  }

  return [`${book.title}`, ...chapters.map((document) => `${document.title}\n\n${htmlToPlainText(document.content)}`)].join(
    "\n\n------------------------------\n\n",
  );
};

const downloadPdf = (title: string, html: string, fileName: string) => {
  const pdf = createWritingPdf(title, html);
  const blob = pdf.output("blob");
  downloadBlob(blob, fileName);
};

export const exportDocument = (document: WritingDocument, format: ExportFormat) => {
  const fileName = safeFileName(document.title, format);

  if (format === "pdf") {
    downloadPdf(document.title.trim() || "Untitled", document.content, fileName);
    return;
  }

  const content = format === "markdown" ? buildMarkdown(document) : htmlToPlainText(document.content);
  const mimeType = format === "markdown" ? "text/markdown;charset=utf-8" : "text/plain;charset=utf-8";
  downloadBlob(new Blob([content], { type: mimeType }), fileName);
};

export const exportBook = (book: WritingBook, documents: WritingDocument[], format: ExportFormat) => {
  const fileName = safeFileName(book.title, format);
  const chapters = orderedBookDocuments(book, documents);

  if (format === "pdf") {
    const html = chapters
      .map((document) => `<h1>${escapeHtml(document.title.trim() || "Untitled")}</h1>${plainTextToHtml(document.content)}`)
      .join("");
    downloadPdf(book.title.trim() || "Untitled", html, fileName);
    return;
  }

  const content = buildBookContent(book, documents, format);
  const mimeType = format === "markdown" ? "text/markdown;charset=utf-8" : "text/plain;charset=utf-8";
  downloadBlob(new Blob([content], { type: mimeType }), fileName);
};
